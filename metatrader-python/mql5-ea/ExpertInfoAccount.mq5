      ...
      if(checkTimeMessage()){
         ...
         do{
            if (glbClientSocket.IsSocketConnected()) {
               ...
               res = glbClientSocket.Send(strMsg);
               break;
            }else{
               count += 1;
            }     
         ...
         if(res == false || count >= MAX_TRIES){
            Print("Account information not sent.");
            
            Print("Generating a csv file...");
            int fileHandle = FileOpen("account_info_MQL5.csv", FILE_READ|FILE_WRITE|FILE_CSV, "|");
            FileSeek(fileHandle, 0, SEEK_END);
            
            string info = "";
            FileWrite(fileHandle, StringConcatenate(info, accountInfoMessage(), "\n"));
            FileClose(fileHandle);
            
            // Send email.
            //SendMail("Error sending ACCOUNT info. Check ExpertInfoAccount.", "Trade info saved as CSV in MQL Files.");
         }
