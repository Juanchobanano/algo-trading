...
def is_normal(r, level = 0.01):
    """
    Applies the Jarque-Bera test to termine if a Series is normal or not 
    Test is applied at the 1% level by default
    Returns True if the hyphotesis of normality is accepted, False otherwise
    """
...
def cvar_historic(r, level = 5):
    """
    Computes te Conditional VaR of Series or DataFrame
    """
...
def get_ind_returns():
    """
    Load and format the Ken French 30 Industry Portfolios Vlaue Weighted Monthly Returns
    """
...
def annualize_rets(r, periods_per_year):
    ...
    but that is currently left as an execise
    to the read
    """
...
def plot_ef(n_points, er, cov, style = ".-", show_cml = False, riskfree_rate = 0, show_ew = False, show_gmv = False):
    ...
    if show_ew:
        ...
        # Display EW (Equaly weighted portfolio)
...
def gmv(cov):
    """
    Returns the weight of the Global Minimum Vol portfolio given the covariance matrix.
    """
    n = cov.shape[0]
